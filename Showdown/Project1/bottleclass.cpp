...
BottleClass::BottleClass(const BottleClass&)
{
}
...
int* BottleClass::ReturnBottleHit()
{
	return bottleHit;
}

std::vector<XMFLOAT3> BottleClass::ReturnBottleVertPosArray()
{
	return bottleVertPosArray;
}

std::vector<DWORD> BottleClass::ReturnBottleVertIndexArray()
{
	return bottleVertIndexArray;
}

XMMATRIX* BottleClass::ReturnBottleWorld()
{
	return bottleWorld;
}

int BottleClass::ReturnNumBottles()
{
	return numBottles;
}

void BottleClass::CleanUp()
{
...
void BottleClass::InitScene(ID3D11Device* d3d11Device, IDXGISwapChain* swapChain)
{
	if (!LoadObjModel(L"bottle.obj", &bottleVertBuff, &bottleIndexBuff, bottleSubsetIndexStart,
		bottleSubsetTexture, material, bottleSubsets, true, true, bottleVertPosArray, bottleVertIndexArray, d3d11Device, swapChain));
...
void BottleClass::DrawScene(ID3D11DeviceContext* context, XMMATRIX camView, XMMATRIX camProjection)
{
	...
	//	context->IASetIndexBuffer(meshIndexBuff, DXGI_FORMAT_R32_UINT, 0);
	//	//Set the grounds vertex buffer
	//	context->IASetVertexBuffers(0, 1, &meshVertBuff, &stride, &offset);
	...
	//	context->PSSetConstantBuffers(1, 1, &cbPerObjectBuffer);
	//	if (material[meshSubsetTexture[i]].hasTexture)
	//		context->PSSetShaderResources(0, 1, &meshSRV[material[meshSubsetTexture[i]].texArrayIndex]);
	//	if (material[meshSubsetTexture[i]].hasNormMap)
	//		context->PSSetShaderResources(1, 1, &meshSRV[material[meshSubsetTexture[i]].normMapTexArrayIndex]);
	...
	for (int j = 0; j < numBottles; j++)
	{
		if (bottleHit[j] == 0)
		{
			...
			{
				//Set the grounds index buffer
				context->IASetIndexBuffer(bottleIndexBuff, DXGI_FORMAT_R32_UINT, 0);
				//Set the grounds vertex buffer
				context->IASetVertexBuffers(0, 1, &bottleVertBuff, &stride, &offset);
				...
				if (material[bottleSubsetTexture[i]].hasTexture)
					context->PSSetShaderResources(0, 1, &meshSRV[material[bottleSubsetTexture[i]].texArrayIndex]);
				if (material[bottleSubsetTexture[i]].hasNormMap)
					context->PSSetShaderResources(1, 1, &meshSRV[material[bottleSubsetTexture[i]].normMapTexArrayIndex]);
				context->PSSetSamplers(0, 1, &CubesTexSamplerState);
	...
	////Set our blend state
	//context->OMSetBlendState(Transparency, NULL, 0xffffffff);
...
bool BottleClass::LoadObjModel
(std::wstring filename, ID3D11Buffer** vertBuff, ID3D11Buffer** indexBuff, 
	std::vector<int>& subsetIndexStart, std::vector<int>& subsetMaterialArray, 
	std::vector<SurfaceMaterial>& material, int& subsetCount, bool isRHCoordSys, 
	bool computeNormals, std::vector<XMFLOAT3>& vertPosArray, std::vector<DWORD>& vertIndexArray, ID3D11Device* d3d11Device, IDXGISwapChain* swapChain)
{
	HRESULT hr = 0;
