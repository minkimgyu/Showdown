...
bool ModelClass::Initialize(ID3D11Device* device, string name, const WCHAR* modelFilename, const WCHAR* textureFilename)
{
	bool result;
...
bool ModelClass::Initialize(ID3D11Device* device, string name, const WCHAR* modelFilename, const WCHAR* textureFilename, ModelData modelData)
{
	bool result;
...
void ModelClass::Frame(float time) {}
...
bool ModelClass::InitializeBuffers(ID3D11Device* device)
{
	...
	{
		XMFLOAT3 position = XMFLOAT3(m_model[i].x, m_model[i].y, m_model[i].z);
		...
		vertPosArray.push_back(position);
		vertIndexArray.push_back(i);
	}
...
void ModelClass::SetName(string name)
{
	this->name = name;
}

ModelData ModelClass::GetModelData()
{
	return m_modelData;
}

XMFLOAT3 ModelClass::SetAngleToEuler(XMFLOAT3 rotation)
{
	return XMFLOAT3(rotation.x * euler, rotation.y * euler, rotation.z * euler);
}

XMMATRIX ModelClass::ReturnModelMatrix()
{
	XMMATRIX xmmatrix = XMMatrixIdentity();

	XMFLOAT3 rotation = SetAngleToEuler(m_modelData.GetRotation());

	xmmatrix *= XMMatrixRotationX(rotation.x);
	xmmatrix *= XMMatrixRotationY(rotation.y);
	xmmatrix *= XMMatrixRotationZ(rotation.z);
	xmmatrix *= XMMatrixScaling(m_modelData.GetScale().x, m_modelData.GetScale().y, m_modelData.GetScale().z);

	xmmatrix *= XMMatrixTranslation(m_modelData.GetPos().x, m_modelData.GetPos().y, m_modelData.GetPos().z);

	return xmmatrix;
}

vector<XMFLOAT3> ModelClass::ReturnVertPosArray()
{
	return vertPosArray;
}

vector<DWORD> ModelClass::ReturnVertIndexArray()
{
	return vertIndexArray;
}
...
XMFLOAT3 ModelClass::GetOffsetPosition()
{
	return m_modelData.GetOffsetPos();
}
